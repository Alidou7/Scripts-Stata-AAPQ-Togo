...
*** Femme ayant des enfant

keep if CM1==1

*** Construction des variable accouchement assisté
gen sba = .
replace sba = 1 if MN19A == "A" | MN19B == "B" | MN19C == "C" | MN19D == "D"  // Inclut accoucheuse auxiliaire
replace sba = 0 if MN19E == "E" | MN19F == "F" | MN19G == "G" | MN19H == "H" | MN19X == "X" | MN19Y == "Y"
...
*** Quintile
gen quintile=.
replace quintile=1 if windex5==1
replace quintile=2 if windex5==2
replace quintile=3 if windex5==3
replace quintile=4 if windex5==4
replace quintile=5 if windex5==5
...
**** Education
gen educatt=. 
replace educatt=1 if welevel==0
replace educatt=2 if welevel==1
replace educatt=3 if welevel==2
label var educatt "Education"
label def educatt 1 "No_education" 2 "Primaire" 3 "Secondary_high"
...
*****varibale regio
gen region=.
replace region=1 if HH7==5
replace region=2 if HH7==3
replace region=3 if HH7==4
replace region=4 if HH7==2
replace region=5 if HH7==1
replace region=6 if HH7==6|HH7==7
...
** retenir les idividu sans données manquante!
keep if sba !=.


**** sauver la nouvelle base
keep WM1 stratum weight sba region quintile urban mage educatt
save base_MIC_2017, replace


*** Calcule des estimation pour preparer la fiche excel

use base_MIC_2017, replace
svyset WM1 [pweight=weight], strata(stratum)
...
save "sample_data_17_disaggregated.dta", replace
...
// 	Declarons les données d'enquête 
	svyset WM1 [pweight=weight], strata(stratum) singleunit(centered)
...
save "sample_data_17_population.dta", replace


***Exportons l'estimation nationale
************************************************************
// utilison la base de donnée 2017
	use base_MIC_2017, replace
...
//	Save national average estimates into a tempfile
	...
	parmest, idstr(national) list(,) saving("sample_data_17_national.dta", replace)
...
//	Collapse dataset into a dataset of sum of the count of records
	...
	save "sample_data_17_samplesize.dta", replace
			 

	
// Renommer la variable « estimate » de l'ensemble de données sur la moyenne nationale en « setting average »

    use "sample_data_17_national.dta", clear
	rename estimate setting_average
	tempfile sample_data_17_national
	save `sample_data_17_national', replace
...
// Renommer la variable « estimate » de l'ensemble de données « population » en « population ».
	use "sample_data_17_population.dta", clear
	rename estimate population
	tempfile sample_data_17_population
	save `sample_data_17_population', replace
	
//	Reshape sample size data to long format
// Transformons les données de la taille de l'échantillon en format long
	use "sample_data_17_samplesize.dta", clear
	rename * samplesize*
	gen parm="sba"
	reshape long samplesize, i(parm) j(idstr) string
	tempfile sample_data_17_samplesize
	save `sample_data_17_samplesize', replace

// 	Open the dataset with disaggregated data
	use "sample_data_17_disaggregated.dta", clear
	
//	Merge national average, population size and sample size data
	merge m:1 parm using `sample_data_17_national', nogen
	merge 1:1 idstr using `sample_data_17_population', nogen
	merge 1:1 idstr using `sample_data_17_samplesize', nogen
...
/// Creeons la varible specific a HEAT plus
	...
	gen reference_subgroup=0
		replace reference_subgroup=1 if inlist(subgroup,"Secondary_high","Urban", "Lomé_commune")
...
//	exporetre les donné en excel
	export excel using "dhs_17.xlsx", firstrow(var) replace
