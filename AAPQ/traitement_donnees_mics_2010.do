...
***** Retenir les femmes ayant deja un enfant vivant
keep if CM1

**** Age des dernier naissances

gen age_nais= WM6Y-CM12Y
...
replace sba=0 if MN17G== "G" |MN17H== "H" |MN17X== "X" |MN17Y== "Y"|MN17Y=="F"
...
**** Education
gen educatt=1 if  welevel==1
replace educatt=2 if  welevel==2
replace educatt=3 if  welevel==3
label var educatt "Ecationnal"
...
keep if sba !=. & weight !=.
keep  WM1 strate weight sba sba region quintile urban mage educatt
save base_mics_2010, replace
...
*** Calcule des estimation pour preparer la fiche exce
*** declaration d'une enquete
use base_mics_2010, replace
svyset WM1 [pweight=weight], strata(strate) singleunit(centered)
...
save "sample_data_10_disaggregated.dta", replace
...
save "sample_data_10_population.dta", replace


***Exportons l'estimation nationale
************************************************************
// utilison la base de donnée 2010
	use base_mics_2010, replace
...
//	Save national average estimates into a tempfile
	...
	parmest, idstr(national) list(,) saving("sample_data_10_national.dta", replace)
...
//	Collapse dataset into a dataset of sum of the count of records
	...
	save "sample_data_10_samplesize.dta", replace
			 

	
// Renommer la variable « estimate » de l'ensemble de données sur la moyenne nationale en « setting average »

    use "sample_data_10_national.dta", clear
	rename estimate setting_average
	tempfile sample_data_10_national
	save `sample_data_10_national', replace
...
// Renommer la variable « estimate » de l'ensemble de données « population » en « population ».
	use "sample_data_10_population.dta", clear
	rename estimate population
	tempfile sample_data_10_population
	save `sample_data_10_population', replace
	
//	Reshape sample size data to long format
// Transformons les données de la taille de l'échantillon en format long
	use "sample_data_10_samplesize.dta", clear
	rename * samplesize*
	gen parm="sba"
	reshape long samplesize, i(parm) j(idstr) string
	tempfile sample_data_10_samplesize
	save `sample_data_10_samplesize', replace

// 	Open the dataset with disaggregated data
	use "sample_data_10_disaggregated.dta", clear
	
//	Merge national average, population size and sample size data
	merge m:1 parm using `sample_data_10_national', nogen
	merge 1:1 idstr using `sample_data_10_population', nogen
	merge 1:1 idstr using `sample_data_10_samplesize', nogen
...
//	exporetre les donné en excel
	export excel using "dhs_10.xlsx", firstrow(var) replace
